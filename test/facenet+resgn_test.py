...
class InceptionResnetV1Model(Model):
    ...
    def __init__(self):
        ...
        self.net_g = SRGAN_g(self.t_image, is_train=False, reuse=tf.AUTO_REUSE)
        image_batch1 = (self.net_g.outputs + 1)/2
        image_batch1 = tf.identity(image_batch1, 'input')
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        tl.files.load_and_assign_npz(sess=sess,
                                     name='E:\\models\\GAN\\g_srgan_softmax50.npz',
                                     network=model.net_g)
