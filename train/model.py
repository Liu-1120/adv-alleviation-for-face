...
def SRGAN_g(t_image, is_train=True, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        n = Conv2d(n, 64, (3, 3), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='n256s1/c')
        temp = n

        # B residual blocks
        for i in range(8):
            nn = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n256s1/c1/%s' % i)
            nn = BatchNormLayer(nn, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n256s1/b1/%s' % i)
            nn = Conv2d(nn, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n256s1/c2/%s' % i)
            nn = BatchNormLayer(nn, is_train=is_train, gamma_init=g_init, name='n256s1/b2/%s' % i)
            nn = ElementwiseLayer([n, nn], tf.add, name='b_residual_add/%s' % i)
            n = nn

        n = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n256s1/c/m')
        n = BatchNormLayer(n, is_train=is_train, gamma_init=g_init, name='n256s1/b/m')
        n = ElementwiseLayer([n, temp], tf.add, name='add3')
        # B residual blacks end
        # n = DeConv2d(n, 64, (1, 1), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c/d1')
        # n = DeConv2d(n, 64, (1, 1), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c/d2')
        # n = Conv2d(n, 256, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, name='n256s1/1')
        # n = SubpixelConv2d(n, scale=2, n_out_channel=None, act=tf.nn.relu, name='pixelshufflerx2/1')
        # n = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c/d')
...
def SRGAN_g_dense(t_image, is_train=False, reuse=False):
    """ Generator in Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network
    feature maps (n) and stride (s) feature maps (n) and stride (s)
    """
    w_init = tf.random_normal_initializer(stddev=0.02)
    b_init = None  # tf.constant_initializer(value=0.0)
    g_init = tf.random_normal_initializer(1., 0.02)
    with tf.variable_scope("SRGAN_g_dense", reuse=reuse) as vs:
        # tl.layers.set_name_reuse(reuse) # remove for TL 1.8.0+
        n = InputLayer(t_image, name='in')
        n = Conv2d(n, 64, (3, 3), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='n256s1/c')
        # temp = n
        n = MaxPool2d(n, (3, 3), (2, 2), padding='SAME', name='maxpools1')
        # B dense blocks
        for i in range(8):
            #dense layer1
            nn = BatchNormLayer(n, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n256s1/b1/%s' % i)
            nn1 = Conv2d(nn, 128, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                        name='n256s1/c1/%s' % i)
            #dense layer2
            nn = BatchNormLayer(nn1, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n256s1/b2/%s' % i)
            nn2 = Conv2d(nn, 128, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                        name='n256s1/c2/%s' % i)
            print(nn1)
            #dense layer3
            # tf.concat([nn1, nn2], 0)
            nn = tl.layers.ConcatLayer([nn1, nn2], 3, name='concat_layer3/%s' % i)
            # nn = ElementwiseLayer([nn1, nn1], tf.concat, name='b_residual_add3/%s' % i)
            nn = BatchNormLayer(nn, is_train=is_train, gamma_init=g_init, name='n256s1/b3/%s' % i)
            nn3 = Conv2d(nn, 128, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                         name='n256s1/c3/%s' % i)
            # dense layer4
            nn = tl.layers.ConcatLayer([nn1, nn2], 3, name='concat_layer4_1/%s' % i)
            nn = tl.layers.ConcatLayer([nn, nn3], 3, name='concat_layer4_2/%s' % i)
            # nn = ElementwiseLayer([nn1, nn2], tf.concat, name='b_residual_add4_1/%s' % i)
            # nn = ElementwiseLayer([nn, nn3], tf.concat, name='b_residual_add4_2/%s' % i)
            nn = BatchNormLayer(nn, is_train=is_train, gamma_init=g_init, name='n256s1/b4/%s' % i)
            nn4 = Conv2d(nn, 128, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                         name='n256s1/c4/%s' % i)
            # dense layer5
            nn = tl.layers.ConcatLayer([nn1, nn2], 3, name='concat_layer5_1/%s' % i)
            nn = tl.layers.ConcatLayer([nn, nn3], 3, name='concat_layer5_2/%s' % i)
            nn = tl.layers.ConcatLayer([nn, nn4], 3, name='concat_layer5_3/%s' % i)
            # nn = ElementwiseLayer([nn1, nn2], tf.concat, name='b_residual_add5_1/%s' % i)
            # nn = ElementwiseLayer([nn, nn3], tf.concat, name='b_residual_add5_2/%s' % i)
            # nn = ElementwiseLayer([nn, nn4], tf.concat, name='b_residual_add5_3/%s' % i)
            nn = BatchNormLayer(nn, is_train=is_train, gamma_init=g_init, name='n256s1/b5/%s' % i)
            nn5 = Conv2d(nn, 128, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                         name='n256s1/c5/%s' % i)



            n = BatchNormLayer(nn5, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n256s1/b/m/%s' % i)
            n = Conv2d(n, 128, (1, 1), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                       name='n256s1/c/m/%s' % i)
            n = MeanPool2d(n, (2, 2), (2, 2), padding='SAME', name='meanpools1/%s' % i)


        n = Conv2d(n, 64, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c/d')

        n = Conv2d(n, 3, (1, 1), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='out')
        return n
