    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                ...
                print(net)
    return net, t_feature1, t_feature


def inception_resnet_v1_1(inputs, is_training=True,
                        dropout_keep_prob=0.8,
                        bottleneck_layer_size=128,
                        reuse=None,
                        scope='InceptionResnetV1'):
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training):
            net, end_points = inception_resnet_v1(inputs, is_training=True,
                                dropout_keep_prob=0.8,
                                bottleneck_layer_size=128,
                                reuse=None,
                                scope=scope)
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                                stride=1, padding='SAME'):
                aux_logits = end_points['Mixed_8b']
                with tf.variable_scope('AuxLogits'):
                    aux_logits = slim.avg_pool2d(aux_logits, aux_logits.get_shape()[1:3], padding='VALID',
                                          scope='AvgPool_1a_8x8')
                    aux_logits = slim.conv2d(
                        aux_logits, 256, 3, stride=2, padding='VALID',scope='Conv2d_4b_3x3')
